...
const functionGetName = {
    data: models_1.getName,
    subData: models_1.getNameSubData,
    additionalFields: models_1.getNameAdditionalFields,
    paymentSpecific: models_1.getNamePaymentSpecific,
};
...
function addMetaData(emvItem) {
    ...
    if (emvItem.id == 52) {
        const mcc_code = data_1.data.mcc.find((item) => item.mcc === emvItem.data);
        mcc_code &&
            (emvItem.data = `${emvItem.data} (${mcc_code.usda_description})`);
    }
    if (emvItem.id == 53) {
        const currency = data_1.data.currencies.find((item) => item.number === emvItem.data);
        currency && (emvItem.data = `${currency.code}`);
    }
    if (emvItem.id == 58) {
        const country = data_1.data.countries.find((item) => item.code === emvItem.data);
